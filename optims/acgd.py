...
class ACGD(object):
    ...
    def step(self, loss):
        ...
        if self.state['solve_x']:
            ...
            update_min = cg_y.mul(- lr_min.sqrt())
            hcg = Hvp_vec(grad_y_vec, self.max_params, update_min).detach_()
            hcg.add_(grad_x_vec_d)
            update_max = hcg.mul(lr_max)
            ...
            update_max = cg_x.mul(lr_max.sqrt())
            hcg = Hvp_vec(grad_x_vec, self.min_params, update_max).detach_()
            hcg.add_(grad_y_vec_d)
            update_min = hcg.mul(- lr_min)
        ...
        for p in self.max_params:
            p.data.add_(update_max[index: index + p.numel()].reshape(p.shape))
            index += p.numel()
        assert index == update_max.numel(), 'Maximizer CG size mismatch'

        index = 0
        for p in self.min_params:
            p.data.add_(update_min[index: index + p.numel()].reshape(p.shape))
            index += p.numel()
        assert index == update_min.numel(), 'Minimizer CG size mismatch'

        if self.collect_info:
            norm_gx = torch.norm(grad_x_vec, p=2).item()
            norm_gy = torch.norm(grad_y_vec, p=2).item()
            norm_cgx = torch.norm(update_max, p=2).item()
            norm_cgy = torch.norm(update_min, p=2).item()
